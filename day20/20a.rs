...
fn to_index(b: &[[bool; 106]; 106], x: usize, y: usize) -> usize {
	let mut res = 0;
	for dx in 0..3 {
		for dy in 0..3 {
...
fn main() {
	...
	let mut b = [[false; 106]; 106];
	
	for row in 0..lines.len() - 2 {
		for col in 0..lines[2].len() {
			b[row + 3][col + 3] = lines[row + 2].as_bytes()[col] == b'#';
		}
	}

	// Step 1 - edge positions are all '.'
	let mut b2 = [[true; 106]; 106];
